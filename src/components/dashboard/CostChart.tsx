...
function CustomTooltip({ active, payload }: { active?: boolean; payload?: Array<{ payload: CostHistoryEntry }> }) {
  ...
  return (
    <div className="bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 shadow-xl text-xs">
      ...
      <p className="text-emerald-400 font-mono mt-1">{entry.messages} messages</p>
  ...
  if (loading) {
    return (
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 h-full flex flex-col">
        <h2 className="text-sm font-semibold text-zinc-300 uppercase tracking-wider mb-4">Activity History</h2>
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 h-full flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-semibold text-zinc-300 uppercase tracking-wider">Activity History</h2>
        <span className="text-xs text-zinc-500 font-mono">14-day total: <span className="text-emerald-400">{totalMessages} msgs</span></span>
      </div>
